...
# Alpha values
a <- as.data.frame(matrix(c(2,0.8),1,2))
# Gamma matrix
...
plot(0, 0, xlim = c(0, limX), ylim = c(0, limY),
     type = 'n',xlab = "u1", ylab = "u2", asp=1)
# Plot vectors showing the direction of each point
for(x in seq(0,limX,0.1)){
  for(y in seq(0,limY,0.1)){
...
initPoints <- data.frame(y1 = c(0.01, 0.2,0.01,3),  # u1 values
                         y2 = c(0.5, 0.01,1.6,0.25),   # u2 values
                         cols = c("green", "orange", "blue","brown")) # Colors
...
for(p in 1:nrow(initPoints)){
  ...
  points(sol2[1,'y1'], sol2[1,'y2'], pch = 19, col = initPoints[p,3], cex=2)
}
